...
def parse_args():
    parser = argparse.ArgumentParser(description='Deep Learning based colorization')
    parser.add_argument('--input', help='Path to input video. Skip to capture frames from camera')
    ...
    parser.add_argument('--savefile', help='Output Video location', default="out.mp4",type=str,required=False )
...
if __name__ == '__main__':
    ...
    WRITE_VIDEO_FLAG=args.savefile
    if WRITE_VIDEO_FLAG:
    # Define the codec and create VideoWriter object
    w = int(cap.get(4))
    h = int(cap.get(3))
    fourcc = cv.VideoWriter_fourcc(*'MP4V')
    writer = cv.VideoWriter(args.savefile, fourcc, 25, imshowSize)
    ...
    while cv.waitKey(1) < 0:
        ...
        img_bgr_out = 255 * np.clip(cv.cvtColor(img_lab_out, cv.COLOR_Lab2BGR), 0, 1)
        img_bgr_out = np.uint8(img_bgr_out)
        con = np.hstack([frame,img_bgr_out]) 
        # Save the output to video depending on input parameters.
        writer.write(cv.resize(con,imshowSize))